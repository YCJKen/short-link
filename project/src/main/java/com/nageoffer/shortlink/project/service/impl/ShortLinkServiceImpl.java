...
public class ShortLinkServiceImpl extends ServiceImpl<ShortLinkMapper, ShortLinkDO> implements ShortLinkService {
    ...
    public ShortLinkCreateRespDTO createShortLink(ShortLinkCreateReqDTO requestParam) {
        ...
        // 该操作往缓存中添加键值对，完整短链接作为键，原始url作为值，并设置了缓存的有效期
        stringRedisTemplate.opsForValue().set(
                String.format(GOTO_SHORT_LINK_KEY, fullShortUrl),
                requestParam.getOriginUrl(),
                LinkUtil.getLinkCacheValidTime(requestParam.getValidDate()), TimeUnit.MILLISECONDS
        );
        // 往布隆过滤器中添加完整短链接防止重复添加
    ...
    public void updateShortLink(ShortLinkUpdateReqDTO requestParam) {
        ...
        if (Objects.equals(hasShortLinkDO.getGid(), requestParam.getGid())) {
            ...
            // 获取读写锁对象，并为对写锁对象赋予唯一标识名称，因为redis中以键值对的形式存在，方便后期并发访问时能够根据唯一标识进行锁对象的获取
            RReadWriteLock readWriteLock = redissonClient.getReadWriteLock(String.format(LOCK_GID_UPDATE_KEY, requestParam.getFullShortUrl()));
            // 从读写锁对象中获取写锁，读写锁时redisson中封装的锁类型，读锁允许多线程同时访问，写锁只允许单一线程进行访问
            RLock rLock = readWriteLock.writeLock();
            if (!rLock.tryLock()) {
                // 尝试获取写锁，如果获取失败则证明已经有其他线程正在访问
                throw new ServiceException("短链接正在被访问，请稍后再试...");
            }
            try {
                // 使用try, final能够保证无论程序执行成功与否都能够释放锁
    ...
    public void shortLinkStats(String fullShortUrl, String gid, ShortLinkStatsRecordDTO statsRecord) {
        ...
        if (!rLock.tryLock()) {
            //如果没有获取读锁，则将访问记录实体发送给延迟队列，这样可以避免以下情况发生
            // 如果一个线程获取的写锁，其他线程无法获取读锁也就意味着无法访问短链接，如果没有延迟队列进行访问信息的处理，那么这么无法对短链接进行访问的线程
            // 的访问信息就无法被记录，
            // ***添加延迟队列后，虽然这些线程无法访问短链接，但是访问记录会被提交到延迟队列当中，生产者会将访问信息提交给消费者队列处理
            // 消费者队列会异步处理访问信息，将访问信息添加进数据库
